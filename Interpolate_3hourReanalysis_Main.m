...
% 2016-10-13: assume porjection system (lat lon in meters); also, consider
% missinging and filling values of reanalysis
...
% VARNAME: meteorological variable name;
...
%% example:
% Interpolate_3hourReanalysis_Main('AQRVPM25',[datenum(2012,1,1),datenum(2012,12,31)],'air.sfc','By-Year','DailyMax',0)

%% code
function ReturnStatus = Interpolate_3hourReanalysis_Main(SITENAME,INPUTDATE,VARNAME,OPTION,DATA_OPTION,EXTRAOPTION,FORMAT)

%% macro definition
GCS = 'North_America_Equidistant_Conic';%% projection system to be used
VARNAME_LIST =  {'air.sfc','dpt.2m','apcp','dlwrf','dswrf','evap','gflux','lhtfl','shtfl','shum.2m','snowc','soilm','tcdc','ulwrf.sfc','uswrf','uwnd.10m','vwnd.10m','weasd','prate','vis','hpbl','hcdc','lcdc','mcdc','pr_wrt','veg'};
VARNAME_FIELD_LIST =  {'air','dpt','apcp','dlwrf','dswrf','evap','gflux','lhtfl','shtfl','shum','snowc','soilm','tcdc','ulwrf','uswrf','uwnd','vwnd','weasd','prate','vis','hpbl','hcdc','lcdc','mcdc','pr_wrt','veg'};
...
% output path
DIRPATHNC_ROOT = '../data/unprocessed/Data_3HourNCEP/';
...
FileName = [DIRPATHNC_ROOT,VARNAME,Sep,VARNAME,'.',num2str(INPUTYEAR),'.nc'];
fprintf('%s\n',FileName);
...
try
    ...
    TempTime = double(ncread(FileName,'time'));% size:2920           1
    TempData = double(ncread(FileName,VARNAME_FIELD));% size: 349*277*2920
    TempData =  Interpolate_3hourReanalysis_function(FileInfo,TempData,VARNAME_FIELD);
catch exception
...
if(~isempty(strfind(DATA_OPTION,'Day')))
    FileName = [DIRPATHNC_ROOT,VARNAME,Sep,VARNAME,'.',num2str(INPUTYEAR-1),'.nc'];
    fprintf('%s\n',FileName);
    try
        FileInfo = ncinfo(FileName);
        TempTime_1 = double(ncread(FileName,'time'));
        TempData_1 = double(ncread(FileName,VARNAME_FIELD));
...
if(exist(WeightFile,'file'))
    ...
    if(exist(CurrentFile1,'file'))
        % read mat file data; if false, return error 
...
TempResult =  Interpolate_3hourReanalysis_function(FileInfo,TempResult,VARNAME_FIELD);
...
if(~isempty(strfind(DATA_OPTION,'Day')))
    ...
    TemporalSize = yeardays(INPUTYEAR)+yeardays(INPUTYEAR-1);
    ...
    % take only this year...
    TempResult = TempResult((yeardays(INPUTYEAR-1)+1):(yeardays(INPUTYEAR)+yeardays(INPUTYEAR-1)),:);
end

%% interpolation and save output
if(strcmp(OPTION,'By-Year'))  
    Result = nan(yeardays(INPUTYEAR),N_Traget);
%     Result1 = InterpMyData_2(TempResult,Lon_Query,Lat_Query,Lon_Target,Lat_Target,'');%too slow...
